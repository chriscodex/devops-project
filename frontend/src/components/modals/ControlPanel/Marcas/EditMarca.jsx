...
function EditMarcaModal(props) {
  const { loadItems, itemName, itemId } = props;
  const { updateMarca } = useOrden();
  ...
  const handleSaveButton = async () => {
    try {
      if (inputValue.length === 0) {
        setInputValueError('Ingrese una marca');
        return;
      }
      await updateMarca(itemId, { nombre: inputValue });
      Swal.fire({
        position: 'center',
        icon: 'success',
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        <div className="w-1/3 h-auto bg-[#efeff4] rounded-lg absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 grid grid-cols-1 select-none">
          <div className="w-full col-span-2 px-3 border-b border-b-gray-400 flex text-blue-600 justify-center">
            <h1 className="text-2xl my-3 font-bold">Editar Marca</h1>
