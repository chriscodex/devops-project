...
function EditArea(props) {
  const { ordenId, areaOrden, loadOrden } = props;
  const { areas, getAreas, updateOrden } = useOrden();
  ...
  const [areaSelect, setAreaSelect] = useState(areaOrden);

  useEffect(() => {
    setAreaSelect(areaOrden);
  }, [areaOrden]);

  const handleSelectArea = (event) => {
    const nuevoValor = event.target.value;
    setAreaSelect(nuevoValor);
  };
  ...
  const handleSaveButton = async () => {
    try {
      await updateOrden(ordenId, { area: areaSelect });
      Swal.fire({
        position: 'center',
        icon: 'success',
