...
import { AddCategoriaModal } from '../../../components/modals/ControlPanel/Categorias/AddCategoriaModal';
import { EditCategoriaModal } from '../../../components/modals/ControlPanel/Categorias/EditCategoriaModal';

function CategoriasControl() {
  const { setEquipo, setCliente, getAllCategoriasEquipo, deleteCategoriaEquipo } =
    useOrden();
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const res = await getAllCategoriasEquipo();
      setLoading(false);
      setNuevaLista(res);
    } catch (error) {
      console.log(error);
      ...
      <div className="absolute left-[310px] h-auto w-[calc(100%-310px)] flex flex-col">
        <div className="flex flex-col items-center w-full h-full font-normal">
          ...
          <div className="px-[20px] w-full grid lg:grid-cols-2 md:grid-cols-1 mb-3">
            <div className="ml-1">
              <AddCategoriaModal loadItems={loadItems} />
            </div>
          </div>
