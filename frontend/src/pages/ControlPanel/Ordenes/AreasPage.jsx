...
import { AddAreaModal } from '../../../components/modals/ControlPanel/Areas/AddAreaModal';
import { CircularProgress, Tooltip } from '@mui/material';
import Swal from 'sweetalert2';
import { EditAreaModal } from '../../../components/modals/ControlPanel/Areas/EditAreaModal';

function AreasControl() {
  const { setEquipo, setCliente, getAreas, deleteArea } = useOrden();
  ...
  const handleDeletePrioridad = async (id) => {
    Swal.fire({
      title: '¿Seguro que desea eliminar esta área?',
      icon: 'warning',
      ...
      <div className="absolute left-[310px] h-auto w-[calc(100%-310px)] flex flex-col">
        <div className="flex flex-col items-center w-full h-full font-normal">
          ...
          <div className="px-[20px] w-full grid lg:grid-cols-2 md:grid-cols-1 mb-3">
            <div className="ml-1">
              <AddAreaModal loadAreas={loadAreas} />
            </div>
          </div>
          <div className="w-[calc(100%-50px)] h-auto bg-white rounded-[10px] shadow-md flex flex-col">
            ...
            <div className="font-bold text-gray-500 flex justify-between items-center py-1 pl-[10px]">
              <div className="flex w-[300px] items-center">
                <h2 className="mr-[10px] select-none">LISTA DE ÁREAS</h2>
                {loading ? (
                  ...
                  <Tooltip
                    ...
                    disableInteractive
                  >
                    <span onClick={loadAreas} className="cursor-pointer">
                      <svg
              ...
              <table className="w-full table-auto">
                <thead className="w-full select-none">
                  <tr>
                    ...
                    <th className="border-r px-2">Área</th>
                  ...
                  {nuevaLista?.map((item, index) => {
                    return (
                      <tr key={index} className="h-10 border">
                        ...
                        <td className="text-center px-2">{item?.area}</td>
                          ...
                          <div className="flex items-center justify-center gap-2">
                            ...
                            >
                              <span
                                ...
                                onClick={() => handleDeletePrioridad(item._id)}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  fill="none"
