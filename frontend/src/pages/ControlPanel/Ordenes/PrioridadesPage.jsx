...
import { AddPrioridadModal } from '../../../components/modals/ControlPanel/Prioridad/AddPrioridadModal';
import { CircularProgress, Tooltip } from '@mui/material';
import Swal from 'sweetalert2';
import { EditPrioridadModal } from '../../../components/modals/ControlPanel/Prioridad/EditPrioridadModal';

function PrioridadesControl() {
  const { setEquipo, setCliente, getPrioridades, deletePrioridad } = useOrden();
  ...
  const loadPrioridades = async () => {
    try {
      setLoading(true);
      const res = await getPrioridades();
      setLoading(false);
      setNuevaLista(res);
    } catch (error) {
      console.log(error);
  ...
  const handleDeletePrioridad = async (id) => {
    Swal.fire({
      title: '¿Seguro que desea eliminar esta prioridad?',
      icon: 'warning',
      ...
      <div className="absolute left-[310px] h-auto w-[calc(100%-310px)] flex flex-col">
        <div className="flex flex-col items-center w-full h-full font-normal">
          ...
          <div className="px-[20px] w-full grid lg:grid-cols-2 md:grid-cols-1 mb-3">
            <div className="ml-1">
              <AddPrioridadModal loadPrioridades={loadPrioridades} />
            </div>
          </div>
          <div className="w-[calc(100%-50px)] h-auto bg-white rounded-[10px] shadow-md flex flex-col">
            ...
            <div className="font-bold text-gray-500 flex justify-between items-center py-1 pl-[10px]">
              <div className="flex w-[300px] items-center">
                <h2 className="mr-[10px] select-none">LISTA DE PRIORIDADES</h2>
                {loading ? (
                  ...
                  <Tooltip
                    ...
                    disableInteractive
                  >
                    <span onClick={loadPrioridades} className="cursor-pointer">
                      <svg
                  ...
                  {nuevaLista?.map((item, index) => {
                    return (
                      <tr key={index} className="h-10 border">
                        ...
                        <td className="text-center px-2">{item?.prioridad}</td>
                        <td className="text-center px-2">
                          {item?.prioridad !== 'Alta' ? (
                            <div className="flex items-center justify-center gap-2">
                              <EditPrioridadModal
                                loadPrioridades={loadPrioridades}
                                prioridadId={item?._id}
                                prioridadItem={item?.prioridad}
                              />
                              <Tooltip
                                title="Eliminar"
                                placement="right"
                              ...
                              >
                                <span
                                  ...
                                  onClick={() =>
                                    handleDeletePrioridad(item._id)
                                  }
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
