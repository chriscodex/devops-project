...
import { AddTipoServicioModal } from '../../../components/modals/ControlPanel/TiposServicio/AddTipoServicioModal';
import { EditTipoServicioModal } from '../../../components/modals/ControlPanel/TiposServicio/EditTipoServicioModal';

function TiposServicioControl() {
  const { setEquipo, setCliente, getTiposServicios, deleteTipoServicio } =
    useOrden();
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const res = await getTiposServicios();
      setLoading(false);
      setNuevaLista(res);
    } catch (error) {
      console.log(error);
  ...
  const handleDeleteItem = async (id) => {
    Swal.fire({
      title: '¿Seguro que desea eliminar este tipo de servicio?',
      icon: 'warning',
      ...
      <div className="absolute left-[310px] h-auto w-[calc(100%-310px)] flex flex-col">
        <div className="flex flex-col items-center w-full h-full font-normal">
          ...
          <div className="px-[20px] w-full grid lg:grid-cols-2 md:grid-cols-1 mb-3">
            <div className="ml-1">
              <AddTipoServicioModal loadItems={loadItems} />
            </div>
          </div>
