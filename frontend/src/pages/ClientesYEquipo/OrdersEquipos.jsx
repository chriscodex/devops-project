...
import { generarReporteEquipos } from '../../utils/generarPdfEquipos';
...
function OrdersEquiposPage() {
  ...
  const [categoriaEquipoSelected, setCategoriaEquipoSelected] = useState('');
  ...
  const findByQuery = async () => {
    try {
      let queryFind = {};
      const marcaQuery = queryParameters.get('marca');
      const categoriaSelectedQuery = queryParameters.get('categoriEquipo');
      ...
      if (marcaQuery) {
        queryFind = { ...queryFind, marca: marcaQuery };
        setMarcaSelected(marcaQuery);
      }
      if (categoriaSelectedQuery) {
        queryFind = { ...queryFind, categoriaEquipo: categoriaSelectedQuery };
        setCategoriaEquipoSelected(categoriaSelectedQuery);
      }
      if (estadoQuery) {
  ...
  const handleBuscar = async () => {
    try {
      ...
      if (marcaSelected !== '') {
        if (urlQuery !== '') {
          urlQuery = urlQuery + `&marca=${marcaSelected}`;
        } else {
          urlQuery = `?marca=${marcaSelected}`;
        }
        query = { ...query, marca: marcaSelected };
      }
      if (categoriaEquipoSelected !== '') {
        if (urlQuery !== '') {
          urlQuery = urlQuery + `&categoriaEquipo=${categoriaEquipoSelected}`;
        } else {
          urlQuery = `?categoriaEquipo=${categoriaEquipoSelected}`;
        }
        query = {
          ...query,
          categoriaEquipo: categoriaEquipoSelected,
        };
      }
      if (estadoSelected !== '') {
        if (urlQuery !== '') {
      ...
      if (urlQuery === '') {
        window.history.replaceState({}, '', '/clientes/equipos');
      } else {
  ...
  const asyncPetitions = async () => {
    const [marcas, categoriasEquipo] = await Promise.all([
      getAllMarcas(),
      getAllCategoriasEquipo(),
    ]);
    setMarcas(marcas);
    setCategoriasEquipo(categoriasEquipo);
  };
  ...
  const handleDescargar = async () => {
    ...
    await generarReporteEquipos(response, queryDB);
          ...
          <div className="w-full h-auto bg-white rounded-[10px] shadow-md flex flex-col">
            ...
            <div className="w-full flex flex-wrap justify-between">
              ...
              <div className="flex flex-col justify-center items-center px-2 border mx-2 rounded-lg py-2 mt-2">
                <span className="font-medium mb-2">Categoría</span>
                <select
                  name="areasReporteOrdenes"
                  id=""
                  value={categoriaEquipoSelected}
                  onChange={(e) => setCategoriaEquipoSelected(e.target.value)}
                  className="w-[250px] col-span-3 border border-gray-400 rounded-[5px] py-2"
                >
                  <option value="">Todos</option>
                  {categoriasEquipo?.map((categoriaEquipo) => {
                    return (
                      <option
                        key={categoriaEquipo?._id}
                        value={categoriaEquipo?.nombre}
                      >
