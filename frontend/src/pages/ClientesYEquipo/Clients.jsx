...
import { generarReporteClientes } from '../../utils/generarPdfClientes';
import { Sidebar } from '../../components/common/Sidebar';
import { useOrden } from '../../context/OrdenContext';
import { ordenarAlfabeticamenteClientes } from '../../utils/ordenarAlfabeticamenteClientes';
...
function ClientsPage() {
  const { getClientsByParameters, setEquipo, setCliente } = useOrden();
  ...
  const [dniRucInput, setDniRucInput] = useState('');
  ...
  const findByQuery = async () => {
    try {
      let queryFind = {};
      const dniRucQuery = queryParameters.get('dniRuc');
      const tipoClienteQuery = queryParameters.get('tipoCliente');
      ...
      if (dniRucQuery) {
        setDniRucInput(dniRucQuery);
        if (dniRucQuery.length === 8) {
          queryFind = { ...queryFind, dni: dniRucQuery };
        }
        if (dniRucQuery.length === 11) {
          queryFind = { ...queryFind, ruc: dniRucQuery };
        }
      }
      if (tipoClienteQuery) {
        queryFind = { ...queryFind, tipoCliente: tipoClienteQuery };
        setTipoClienteSelected(tipoClienteQuery);
      }
      ...
      const res = await getClientsByParameters(queryFind, currentPage, elementosPorPagina);
      setLista(ordenarAlfabeticamenteClientes(res));

      setQueryDB(queryFind);

      const totalItems = await getClientsByParameters(queryFind, 1, 0);
  ...
  const handleBuscar = async () => {
    try {
      ...
      if (dniRucInput.length !== 0) {
        if (urlQuery !== '') {
          urlQuery = urlQuery + `&dniRuc=${dniRucInput}`;
        } else {
          urlQuery = `?dniRuc=${dniRucInput}`;
        }
        query = { ...query, dniRuc: dniRucInput };
      }
      if (tipoClienteSelected !== '') {
        if (urlQuery !== '') {
          urlQuery = urlQuery + `&tipoCliente=${tipoClienteSelected}`;
        } else {
          urlQuery = `?tipoCliente=${tipoClienteSelected}`;
        }
        query = { ...query, tipoCliente: tipoClienteSelected };
      }
      ...
      const response = await getClientsByParameters(
        query,
        1,
        elementosPorPagina
      );

      const totalItems = await getClientsByParameters(query, 1, 0);
      setTotalLista(totalItems.length);

      setLista(ordenarAlfabeticamenteClientes(response));
  ...
  const handleDescargar = async () => {
    ...
    const response = await getClientsByParameters(queryDB, 1, 0);

    await generarReporteClientes(ordenarAlfabeticamenteClientes(response), queryDB);
  };
  ...
  const loadLista = async (pagina) => {
    try {
      const res = await getClientsByParameters(
        queryDB,
        pagina,
        elementosPorPagina
      );
          ...
          <div className="w-full h-auto bg-white rounded-[10px] shadow-md flex flex-col">
            ...
            <div className="w-full flex flex-wrap justify-between">
              <div className="flex flex-col justify-center items-center p-2 border rounded-lg m-2">
                <span className="pb-2 font-medium">DNI / RUC</span>
                <div className="flex justify-center items-center mt-2">
                  <input
                    type="text"
                    value={dniRucInput}
                    onChange={(e) => setDniRucInput(e.target.value)}
              ...
              <div className="flex flex-col justify-center items-center px-2 border mx-2 rounded-lg">
                <span className="font-medium mb-2">Tipo de Cliente</span>
                <select
                  name="areasReporteOrdenes"
                  id=""
                  value={tipoClienteSelected}
                  onChange={(e) => setTipoClienteSelected(e.target.value)}
                  ...
                  <table className="w-full table-auto">
                    <thead className="w-full select-none">
                      <tr>
                        ...
                        <th className="border-r px-2">Tipo</th>
                        <th className="border-r px-2">
                          Apellidos y Nombres / Razón Social
                        </th>
                        <th className="border-r px-2">DNI / RUC</th>
                      ...
                      {lista?.map((item, index) => {
                        return (
                          <tr key={index} className="h-10 border">
                            ...
                            <td className="text-center px-2">
                              {item?.tipo === 'persona' ? 'Persona' : 'Empresa'}
                            </td>
                            ...
                            <td className="text-center">
                              {item?.tipo === 'persona' ? item?.dni : item?.ruc}
                            </td>
                            ...
                            <td className="text-center px-2">
                              <div className="flex justify-center items-center gap-1">
                                <span
                                  className="border p-1 rounded-lg bg-gray-100 hover:bg-gray-300 cursor-pointer relative flex"
                                  onClick={() => {
                                    navigate(`/ordenes/cliente/${item?._id}`);
                                  }}
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
